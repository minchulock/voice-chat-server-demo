...
- 지연 없는 기본 Push-to-Talk 버튼과 별도 연속대화 버튼
...
원형 마이크 버튼은 기본 Push-to-Talk입니다. 버튼을 누르는 즉시 녹음을 준비하고 누르는 동안 입력받으며, 놓으면 발화를 종료해 STT API로 전송합니다. 별도의 `연속 대화` 버튼을 누르면 자동 VAD 모드가 시작되어 `세션 종료`를 누를 때까지 다음 Turn을 이어서 듣습니다.
