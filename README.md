...
대화창 아래의 듀얼 액션 도크에서 `누르고 말하기`와 `연속 대화 시작`을 바로 선택할 수 있습니다. `누르고 말하기`는 누르는 동안 입력받고 놓으면 STT API로 전송합니다. `연속 대화 시작`은 자동 VAD 모드로 전환되어 `세션 종료`를 누를 때까지 다음 Turn을 이어서 듣습니다.
