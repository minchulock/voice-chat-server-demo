...
- Push-to-Talk과 연속대화를 나란히 선택하는 듀얼 액션 도크
...
- 요약 음성 자동 재생과 말풍선별 답변 전문 듣기
- 연속대화의 TTS 재생 중 발화 인터럽트와 PTT의 명시적 재생 중단
- PTT에서는 다음 입력 대기, 자동 VAD에서는 Turn 종료 후 자동 Listening
- 실시간 API·스트리밍 로그

## 음성 입력 사용법

대화창 아래의 큰 원형 버튼에서 `누르고 말하기`와 `연속 대화`를 바로 선택할 수 있습니다. `누르고 말하기`는 누르는 동안 입력받고 놓으면 STT API로 전송합니다. `연속 대화`를 누르면 자동 VAD 모드가 시작되고 버튼이 `대화 종료`로 바뀝니다. `대화 종료`를 누르면 진행 중인 입력과 응답을 중단하고 서버 세션까지 정리합니다.

브라우저에서 처음 마이크를 사용할 때는 첫 입력으로 권한만 확인합니다. 권한을 허용해도 즉시 Listening으로 넘어가지 않으며, 안내가 표시된 뒤 버튼을 다시 클릭해야 연속대화가 시작됩니다. Push-to-Talk도 권한 허용 후 다시 누른 시점부터 녹음합니다.

| 동작 | 시작 | 종료 |
|---|---|---|
| 연속대화 | `연속 대화` 버튼 클릭 | 같은 버튼의 `대화 종료` 클릭 |
| 마우스 PTT | 마이크 버튼 누르기 | 버튼 놓기 |
| 터치 PTT | 마이크 버튼 터치 | 손가락 떼기 |
| 키보드 PTT | `Space` 누르고 있기 | `Space` 떼기 |

PTT와 연속대화를 구분하는 대기 타이머는 사용하지 않습니다. 텍스트 입력란이나 설정 필드에 포커스가 있을 때는 Space 단축키가 녹음을 시작하지 않습니다. 포인터 취소 또는 브라우저 포커스 이탈 시에는 녹음을 안전하게 종료합니다.

PTT를 선택하는 순간 예약된 연속대화 Listening은 취소됩니다. 자동 발화 인터럽트는 연속대화에서만 동작하므로 PTT가 자동 VAD로 바뀌지 않습니다. TTS 답변 재생 중 PTT를 누르면 현재 답변만 즉시 멈추며, 첫 음절 유실을 피하기 위해 같은 누름에서 녹음을 시작하지 않습니다. 손을 뗀 뒤 다시 누르고 말하면 새 PTT 입력이 시작됩니다.
...
기본 음성 응답은 Agent가 반환한 `speech_text` 요약본을 자동 재생합니다. 화면 말풍선에는 `display_text` 전문을 표시하며, 자동 음성 재생이 끝나면 말풍선의 `전문 듣기` 버튼이 활성화됩니다. 버튼을 누르면 화면 전문을 문장 단위로 TTS 합성해 순차 재생하고, `재생 중지`를 누르거나 새 PTT 발화를 시작하면 즉시 중단합니다. 자동 VAD 모드에서는 전문 재생 중 Listening을 잠시 멈추고 재생 종료 후 다시 시작합니다.
